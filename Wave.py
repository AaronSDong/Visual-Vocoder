...
class Wave:
    def __init__(self, chorus=ChorusSettings.ChorusSettings(bypass=True),
        ...
        self.playing = False
        self.dry_signal = DrySignal.DrySignal(wave_shape=wave_shape, f=f, max_vol=self.max_vol_left,
    ...
    def __init__create_chorus_waves(self):
        ...
        self.lower_chorus['signal'] = DrySignal.DrySignal(wave_shape=self.wave_shape, mono=self.mono,
        ...
        self.upper_chorus['signal'] = DrySignal.DrySignal(wave_shape=self.wave_shape, mono=self.mono,
    ...
    def play_loop(self, t=0.0):
        ...
        while (infinite_flag or time.time() - start_time < t) and self.playing:
            ...
            if (not self.chorus.bypass) and (time.time() - start_time > self.chorus.delay_sec):
                # print(self.lower_chorus['signal'].f)
    ...
    def set_volume(self, volume, channel='mono'):
        if self.mono or channel.lower() in ('left', 'mono'):
            ...
            if not self.chorus.bypass:
                self.lower_chorus['signal'].set_volume(self.max_vol_right, channel='left')
                self.upper_chorus['signal'].set_volume(self.max_vol_right, channel='left')

        if self.mono or channel.lower() in ('right', 'mono'):
            self.max_vol_right = volume
            self.dry_signal.set_volume(self.max_vol_left, channel='right')
            if not self.chorus.bypass:
    ...
    def change_chorus_target_frequency(self, signal):
        ...
        if lower_voice_flag == 1: print(signal['signal'].target_f)
        if signal['decrease_f'] and (signal['signal'].f <= signal['target_f']):
            self.flip_chorus_direction(signal, lower_voice_flag, decrease_f=False)

        elif (not signal['decrease_f']) and signal['signal'].f >= signal['target_f']:\
            self.flip_chorus_direction(signal, lower_voice_flag, decrease_f=True)

    def flip_chorus_direction(self, signal, lower_voice_flag, decrease_f):
        ...
        # if lower_voice_flag == 1: print(signal['target_f'], self.chorus.delay_f, self.chorus.depth_f)
